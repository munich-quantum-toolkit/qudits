...
class Rx(Gate):
    def __init__(
        ...
        if self.validate_parameter(parameters):
            ...
            self.theta = regulate_theta(self.theta)  # Normalisiert auf [0, 2π)
    ...
    def __array__(self) -> NDArray[np.complex128]:  # noqa: PLW3201
        ...
        # Define the three R gates:
        # 1. R(lev_a, lev_b, -π/2, 0) → "X-like" rotation (π/2 around Y)
        pi_there = R(
            self.parent_circuit, "R", qudit_targeted, [self.lev_a, self.lev_b, -np.pi / 2, 0.0], dimension
        ).to_matrix()

        # 2. R(lev_a, lev_b, theta, 0) → Rotation by theta in the X-plane
        rotate = R(
            self.parent_circuit, "R", qudit_targeted, [self.lev_a, self.lev_b, theta, 0.0], dimension
        ).to_matrix()

        # 3. R(lev_a, lev_b, π/2, 0) → Back to original basis
        pi_back = R(
            self.parent_circuit, "R", qudit_targeted, [self.lev_a, self.lev_b, np.pi / 2, 0.0], dimension
        ).to_matrix()
